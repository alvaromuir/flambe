...
(function() {
  var Post, User, checkEmailExists, checkUserName, count, create, db, find, findById, findOne, remove, removeById, removeOne, removeQuick, update, updateById, updateOne, _;
  ...
  findById = function(model, id, fields, opt, cb) {
    return model.findById(id, fields, opt, function(err, rslt) {
      return cb(err, rslt);
    });
  };

  findOne = function(model, criteria, fields, opt, cb) {
    return model.findOne(criteria, fields, opt, function(err, rslt) {
      return cb(err, rslt);
    });
  };
  ...
  checkUserName = function(usrName, cb, orig) {
    return User.findOne({
      userName: usrName
    }, null, null, function(err, rslt) {
      var userName;
      if (rslt) {
        orig = usrName;
        userName = orig + _.random(1, 9999);
        return checkUserName(userName, cb, orig);
      } else {
        return cb(usrName);
      }
    });
  ...
  checkEmailExists = function(email, cb) {
    return User.findOne({
      email: email
    }, null, null, function(err, rslt) {
      if (rslt) {
  ...
  module.exports = {
    ...
    User: {
      ...
      readById: function(id, fields, opt, cb) {
        return findById(User, id, fields, opt, cb);
      },
      readOne: function(criteria, fields, opt, cb) {
        return findOne(User, criteria, fields, opt, cb);
      },
      ...
      suggestUserName: function(usrName, cb) {
        return checkUserName(usrName, cb);
      },
      checkExistingEmail: function(email, cb) {
        return checkEmailExists(email, cb);
      },
      findOrCreateUserByTwitterData: function(data, promise) {
        return findOne(User, {
          ...
          if (rslt) {
            ...
            return checkUserName(suggUserName, function(suggestion) {
              rslt = {
                email: '',
                name: data.name,
                userName: suggestion,
